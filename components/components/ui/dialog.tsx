...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-[9998] bg-black/10 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
  />
  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onPointerDownOutside={(event) => {
        // Support click outside to dismiss
        if (props.onPointerDownOutside) {
          props.onPointerDownOutside(event);
        }
      }}
      onInteractOutside={(event) => {
        // Handle interactions outside the dialog
