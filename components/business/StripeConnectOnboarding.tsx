    ...
    void (async () => {
      ...
      if (onboarding === 'complete') {
        toast({
          ...
          description: 'Stripe Connect is ready. You can submit marketplace products for sale.',
        });
      }
  ...
  const startOnboarding = async () => {
    ...
    try {
      const returnPath = businessId
        ? STRIPE_CONNECT_RETURN_PATHS.marketplace
        : STRIPE_CONNECT_RETURN_PATHS.business;

      const { data, error } = await supabase.functions.invoke('manage-stripe-account', {
        body: { action: 'create_onboarding_link', return_path: returnPath },
      });
        ...
        {stripeAccount?.status === 'no_account' && (
          <>
            <Alert>
              ...
              <AlertDescription>
                Set up your Stripe account to receive payments from marketplace product sales.
              </AlertDescription>
            </Alert>
            
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                Connect your bank account to receive payments securely through Stripe. DogQuest will deduct a €1 commission per sale.
              </p>
              <Button 
          ...
          <>
            ...
            {(connectReady || stripeAccount.payout_enabled || stripeAccount.charges_enabled) && (
              <Alert>
                ...
                <AlertDescription>
                  ✅ You can accept marketplace payments. Payouts to your bank follow Stripe&apos;s verification (€1 commission per sale).
                </AlertDescription>
              </Alert>
            )}
          </>
