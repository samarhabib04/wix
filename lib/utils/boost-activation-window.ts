/**
 * Boost activation windows for listing boosts (`public.boosts`).
 * Keep in sync with `supabase/functions/_shared/boost-activation-window.ts`.
 *
...
const DUBLIN_FORMAT: Intl.DateTimeFormatOptions = {
...
export function formatGoldBoostScheduleMessage(referenceDate = new Date()): string {
  ...
  const endLabel = boostEnd
    ? new Date(boostEnd).toLocaleString('en-IE', DUBLIN_FORMAT)
    : '11:00 PM';
  ...
  return `Your Gold Boost is scheduled for ${start.toLocaleString('en-IE', DUBLIN_FORMAT)} and runs until ${endLabel} that evening (Irish time).`;
}
...
export function isBoostScheduledForFuture(
  boostType: string,
  referenceDate = new Date(),
): boolean {
  const { boostStart } = getBoostActivationWindow(boostType, referenceDate);
  return new Date(boostStart).getTime() > referenceDate.getTime() + 60_000;
}
