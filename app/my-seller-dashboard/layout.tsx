...
import SellerDashboardLayoutNext from '@/components/seller-dashboard/SellerDashboardLayoutNext';
...
export default function SellerDashboardLayoutWrapper({
  ...
  useEffect(() => {
    ...
    // STRICT: If role is loaded and user doesn't have seller role
    if (user && role !== null && role !== 'seller') {
  ...
  useEffect(() => {
    ...
    const tried = sessionStorage.getItem('seller_dashboard_auth_recovery');
    if (tried === '1') return;
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (session?.user) {
        sessionStorage.setItem('seller_dashboard_auth_recovery', '1');
  ...
  useEffect(() => {
    if (user && typeof window !== 'undefined') {
      sessionStorage.removeItem('seller_dashboard_auth_recovery');
    }
  }, [user]);
  ...
  if (user) {
    return <SellerDashboardLayoutNext>{children}</SellerDashboardLayoutNext>;
  }
